...
class DeviceInfo {
    ...
    private String getFacebookAttributionId(Context context) {
        ...
        try {
            if (Build.VERSION.SDK_INT >= 28) {
                signingInfo = context.getPackageManager().getPackageInfo("com.facebook.katana", faceunity.FUAITYPE_FACEPROCESSOR_FACEID).signingInfo;
    ...
    public void reloadPlayIds(AdjustConfig adjustConfig) {
        ...
        if (Util.canReadPlayIds(adjustConfig)) {
            ...
            for (int i = 1; i <= 3; i++) {
                try {
                    GooglePlayServicesClient.GooglePlayServicesInfo googlePlayServicesInfo = GooglePlayServicesClient.getGooglePlayServicesInfo(context, i * BannerConfig.LOOP_TIME);
