...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    public static final class StreamReader implements Reader {
        ...
        /* renamed from: is */
        private final InputStream f7064is;

        public StreamReader(InputStream inputStream) {
            this.f7064is = inputStream;
        }
        ...
        public int read(byte[] bArr, int i) throws IOException {
            int i2 = 0;
            int i3 = 0;
            while (i2 < i && (i3 = this.f7064is.read(bArr, i2, i - i2)) != -1) {
                i2 += i3;
            }
            if (i2 == 0 && i3 == -1) {
                throw new Reader.EndOfFileException();
        ...
        public long skip(long j) throws IOException {
            ...
            long j2 = j;
            while (j2 > 0) {
                long skip = this.f7064is.skip(j2);
                if (skip <= 0) {
                    if (this.f7064is.read() == -1) {
                        break;
                    }
                    skip = 1;
                }
    ...
    private int moveToExifSegmentAndGetLength(Reader reader) throws IOException {
        ...
        if (Log.isLoggable(TAG, 3)) {
            StringBuilder m58818p = yv2.m58818p("Unable to skip enough data, type: ", uInt8, ", wanted to skip: ", uInt16, ", but actually skipped: ");
            m58818p.append(skip);
            Log.d(TAG, m58818p.toString());
        }
        return -1;
    }
            ...
            if (int163 == ORIENTATION_TAG_TYPE) {
                ...
                if (int164 >= 1 && int164 <= 12) {
                    ...
                    if (int322 < 0) {
                        ...
                        if (Log.isLoggable(TAG, 3)) {
                            StringBuilder m58818p = yv2.m58818p("Got tagIndex=", i, " tagType=", int163, " formatCode=");
                            m58818p.append((int) int164);
                            m58818p.append(" componentCount=");
                            m58818p.append(int322);
                            Log.d(TAG, m58818p.toString());
                        }
