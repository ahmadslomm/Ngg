...
public final class AutoSize {
    ...
    public static /* synthetic */ class C40511 {
    ...
    public static void autoConvertDensity(Activity activity, float f, boolean z) {
        ...
        int round = Math.round(AutoSizeConfig.getInstance().getInitScaledDensity() * (f + designWidth + (z ? AutoSizeConfig.getInstance().getScreenWidth() : AutoSizeConfig.getInstance().getScreenHeight())));
        int i = z ? 1073741824 | (round & 1073741823) : round & 1073741823;
    ...
    public static void cancelAdapt(Activity activity) {
        ...
        int i = C40511.$SwitchMap$me$jessyan$autosize$unit$Subunits[AutoSizeConfig.getInstance().getUnitsManager().getSupportSubunits().ordinal()];
        if (i != 1) {
            f = i == 2 ? 25.4f : 72.0f;
    ...
    public static void initCompatMultiProcess(Context context) {
        context.getContentResolver().query(Uri.parse(CommonUtility.PREFIX_URI + context.getPackageName() + ".autosize-init-provider"), null, null, null, null);
    }
    ...
    private static void setDensity(DisplayMetrics displayMetrics, float f, int i, float f2, float f3) {
        ...
        int i2 = C40511.$SwitchMap$me$jessyan$autosize$unit$Subunits[AutoSizeConfig.getInstance().getUnitsManager().getSupportSubunits().ordinal()];
        if (i2 == 1) {
