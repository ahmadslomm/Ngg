package p000;

/* compiled from: zaffa */
/* loaded from: classes.dex */
public final class vj2 extends dm0 {

    /* renamed from: a */
    public final double[] f43022a;

    /* renamed from: b */
    public final double[][] f43023b;

    /* renamed from: c */
    public final boolean f43024c = true;

    /* renamed from: d */
    public final double[] f43025d;

    public vj2(double[] dArr, double[][] dArr2) {
        int length = dArr.length;
        int length2 = dArr2[0].length;
        this.f43025d = new double[length2];
        this.f43022a = dArr;
        this.f43023b = dArr2;
        if (length2 <= 2) {
            return;
        }
        double d = 0.0d;
        int i = 0;
        while (true) {
            double d2 = d;
            if (i >= dArr.length) {
                return;
            }
            double d3 = dArr2[i][0];
            if (i > 0) {
                Math.hypot(d3 - d, d3 - d2);
            }
            i++;
            d = d3;
        }
    }

    @Override // p000.dm0
    /* renamed from: c */
    public double mo13713c(double d, int i) {
        double d2;
        double d3;
        double mo13716f;
        double[] dArr = this.f43022a;
        int length = dArr.length;
        boolean z = this.f43024c;
        double[][] dArr2 = this.f43023b;
        int i2 = 0;
        if (z) {
            double d4 = dArr[0];
            if (d <= d4) {
                d2 = dArr2[0][i];
                d3 = d - d4;
                mo13716f = mo13716f(d4, i);
            } else {
                int i3 = length - 1;
                double d5 = dArr[i3];
                if (d >= d5) {
                    d2 = dArr2[i3][i];
                    d3 = d - d5;
                    mo13716f = mo13716f(d5, i);
                }
            }
            return (mo13716f * d3) + d2;
        }
        if (d <= dArr[0]) {
            return dArr2[0][i];
        }
        int i4 = length - 1;
        if (d >= dArr[i4]) {
            return dArr2[i4][i];
        }
        while (i2 < length - 1) {
            double d6 = dArr[i2];
            if (d == d6) {
                return dArr2[i2][i];
            }
            int i5 = i2 + 1;
            double d7 = dArr[i5];
            if (d < d7) {
                double d8 = (d - d6) / (d7 - d6);
                return (dArr2[i5][i] * d8) + ((1.0d - d8) * dArr2[i2][i]);
            }
            i2 = i5;
        }
        return 0.0d;
    }

    @Override // p000.dm0
    /* renamed from: d */
    public void mo13714d(double d, double[] dArr) {
        double[] dArr2 = this.f43022a;
        int length = dArr2.length;
        double[][] dArr3 = this.f43023b;
        int i = 0;
        int length2 = dArr3[0].length;
        if (this.f43024c) {
            double d2 = dArr2[0];
            double[] dArr4 = this.f43025d;
            if (d <= d2) {
                mo13717g(d2, dArr4);
                for (int i2 = 0; i2 < length2; i2++) {
                    dArr[i2] = ((d - dArr2[0]) * dArr4[i2]) + dArr3[0][i2];
                }
                return;
            }
            int i3 = length - 1;
            double d3 = dArr2[i3];
            if (d >= d3) {
                mo13717g(d3, dArr4);
                while (i < length2) {
                    dArr[i] = ((d - dArr2[i3]) * dArr4[i]) + dArr3[i3][i];
                    i++;
                }
                return;
            }
        } else {
            if (d <= dArr2[0]) {
                for (int i4 = 0; i4 < length2; i4++) {
                    dArr[i4] = dArr3[0][i4];
                }
                return;
            }
            int i5 = length - 1;
            if (d >= dArr2[i5]) {
                while (i < length2) {
                    dArr[i] = dArr3[i5][i];
                    i++;
                }
                return;
            }
        }
        int i6 = 0;
        while (i6 < length - 1) {
            if (d == dArr2[i6]) {
                for (int i7 = 0; i7 < length2; i7++) {
                    dArr[i7] = dArr3[i6][i7];
                }
            }
            int i8 = i6 + 1;
            double d4 = dArr2[i8];
            if (d < d4) {
                double d5 = dArr2[i6];
                double d6 = (d - d5) / (d4 - d5);
                while (i < length2) {
                    dArr[i] = (dArr3[i8][i] * d6) + ((1.0d - d6) * dArr3[i6][i]);
                    i++;
                }
                return;
            }
            i6 = i8;
        }
    }

    @Override // p000.dm0
    /* renamed from: e */
    public void mo13715e(double d, float[] fArr) {
        double[] dArr = this.f43022a;
        int length = dArr.length;
        double[][] dArr2 = this.f43023b;
        int i = 0;
        int length2 = dArr2[0].length;
        if (this.f43024c) {
            double d2 = dArr[0];
            double[] dArr3 = this.f43025d;
            if (d <= d2) {
                mo13717g(d2, dArr3);
                for (int i2 = 0; i2 < length2; i2++) {
                    fArr[i2] = (float) (((d - dArr[0]) * dArr3[i2]) + dArr2[0][i2]);
                }
                return;
            }
            int i3 = length - 1;
            double d3 = dArr[i3];
            if (d >= d3) {
                mo13717g(d3, dArr3);
                while (i < length2) {
                    fArr[i] = (float) (((d - dArr[i3]) * dArr3[i]) + dArr2[i3][i]);
                    i++;
                }
                return;
            }
        } else {
            if (d <= dArr[0]) {
                for (int i4 = 0; i4 < length2; i4++) {
                    fArr[i4] = (float) dArr2[0][i4];
                }
                return;
            }
            int i5 = length - 1;
            if (d >= dArr[i5]) {
                while (i < length2) {
                    fArr[i] = (float) dArr2[i5][i];
                    i++;
                }
                return;
            }
        }
        int i6 = 0;
        while (i6 < length - 1) {
            if (d == dArr[i6]) {
                for (int i7 = 0; i7 < length2; i7++) {
                    fArr[i7] = (float) dArr2[i6][i7];
                }
            }
            int i8 = i6 + 1;
            double d4 = dArr[i8];
            if (d < d4) {
                double d5 = dArr[i6];
                double d6 = (d - d5) / (d4 - d5);
                while (i < length2) {
                    fArr[i] = (float) ((dArr2[i8][i] * d6) + ((1.0d - d6) * dArr2[i6][i]));
                    i++;
                }
                return;
            }
            i6 = i8;
        }
    }

    /* JADX WARN: Code restructure failed: missing block: B:17:0x0012, code lost:
    
        if (r8 >= r3) goto L4;
     */
    @Override // p000.dm0
    /* renamed from: f */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    public double mo13716f(double d, int i) {
        double[] dArr = this.f43022a;
        int length = dArr.length;
        int i2 = 0;
        double d2 = dArr[0];
        if (d >= d2) {
            d2 = dArr[length - 1];
        }
        d = d2;
        while (i2 < length - 1) {
            int i3 = i2 + 1;
            double d3 = dArr[i3];
            if (d <= d3) {
                double d4 = d3 - dArr[i2];
                double[][] dArr2 = this.f43023b;
                return (dArr2[i3][i] - dArr2[i2][i]) / d4;
            }
            i2 = i3;
        }
        return 0.0d;
    }

    /* JADX WARN: Code restructure failed: missing block: B:20:0x0017, code lost:
    
        if (r12 >= r5) goto L4;
     */
    @Override // p000.dm0
    /* renamed from: g */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    public void mo13717g(double d, double[] dArr) {
        double[] dArr2 = this.f43022a;
        int length = dArr2.length;
        double[][] dArr3 = this.f43023b;
        int length2 = dArr3[0].length;
        double d2 = dArr2[0];
        if (d > d2) {
            d2 = dArr2[length - 1];
        }
        d = d2;
        int i = 0;
        while (i < length - 1) {
            int i2 = i + 1;
            double d3 = dArr2[i2];
            if (d <= d3) {
                double d4 = d3 - dArr2[i];
                for (int i3 = 0; i3 < length2; i3++) {
                    dArr[i3] = (dArr3[i2][i3] - dArr3[i][i3]) / d4;
                }
                return;
            }
            i = i2;
        }
    }

    @Override // p000.dm0
    /* renamed from: h */
    public double[] mo13718h() {
        return this.f43022a;
    }
}
