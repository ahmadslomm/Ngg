...
public class PublisherConfiguration {
    ...
    private PublisherConfiguration(Builder builder) {
        try {
            this.jsonObject = new JSONObject().put("owner", builder.params.owner).put("lifecycle", builder.params.lifecycle).put("defaultLayout", builder.params.defaultLayout).put(ViewHierarchyConstants.DIMENSION_WIDTH_KEY, builder.params.width).put(ViewHierarchyConstants.DIMENSION_HEIGHT_KEY, builder.params.height).put("framerate", builder.params.framerate).put("bitrate", builder.params.bitrate).put("mosaicStream", builder.params.publishUrl).put("rawStream", builder.params.rawStreamUrl).put("extraInfo", builder.params.extraInfo);
            if (builder.params.injectStreamUrl == null || builder.params.injectStreamWidth == 0 || builder.params.injectStreamHeight == 0) {
                return;
            }
            this.jsonObject.put("injectInfo", new JSONObject().put("injectStream", builder.params.injectStreamUrl).put(ViewHierarchyConstants.DIMENSION_WIDTH_KEY, builder.params.injectStreamWidth).put(ViewHierarchyConstants.DIMENSION_HEIGHT_KEY, builder.params.injectStreamHeight));
        } catch (JSONException unused) {
            this.jsonObject = null;
            Logging.m23901e("failed to create PublisherConfiguration");
        }
    }
