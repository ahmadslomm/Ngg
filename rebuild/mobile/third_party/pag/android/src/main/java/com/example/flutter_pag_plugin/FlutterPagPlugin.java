...
// [VENDOR PATCH] Removed v1-embedding imports (PluginRegistry, io.flutter.view.FlutterNativeView):
// those classes no longer exist in modern Flutter. Only the v2 embedding path is kept.
...
public class FlutterPagPlugin implements FlutterPlugin, MethodCallHandler {
    ...
    Context context;
    // [VENDOR PATCH] Removed the v1-embedding `registrar` field (Registrar was removed from Flutter).
    ...
    // [VENDOR PATCH] Removed the v1-embedding constructor `FlutterPagPlugin(Registrar)`.
    ...
    // [VENDOR PATCH] Removed the v1-embedding `registerWith(Registrar)` entrypoint. Registration
    // now happens exclusively through the v2 embedding (`onAttachedToEngine`) above.
    ...
    private void initPag(final MethodCall call, final Result result) {
        ...
        if (bytes != null) {
            ...
            String assetKey;
            // [VENDOR PATCH] Dropped the v1 `registrar.lookupKeyForAsset` branch; resolve asset keys
            // exclusively through the v2 embedding's FlutterAssets.
